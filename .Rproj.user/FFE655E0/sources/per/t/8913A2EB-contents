...
bg.picker2 <- function(x,y){
  ifelse(x == y, "#ffff00", ifelse(x >= 6, '#bfff00', ifelse(x < 6 & x >= 5.5 ,"#80ff00",  ifelse(x < 5.5 & x >= 5 ,"#40ff00", ifelse(x < 5 & x >= 4.5, "#40ff00", ifelse(x < 4.5 & x >= 4, '#00ff00', ifelse(x < 4 & x >= 3.5, '#00ff00', ifelse(x < 3.5 & x >= 3, "#ff4000",ifelse(x < 3 & x >= 2.5, "#ff4000", ifelse(x < 2.5 & x >= 2,  "#ff0000", "#ff0000"))))))))))
}
...
<script src="https://kit.fontawesome.com/a076d05399.js"></script>
 ...
 player_radar_1 <- reactive({
   ...
   tes2 <- fpl_bar %>% 
     ...
     dplyr::mutate_if(is.double, funs(round(., 2))) %>% 
...
output$player_pitch <- renderPlot({
 ...
 table <- table %>%
    mutate(
       ...
       `GW 20` = cell_spec(`GW 20`, "html", background = bg.picker2(`Points 20`, max1), color = "white", tooltip = paste0("Predicted Point: ",`Points 20`)),
       `GW 21` = cell_spec(`GW 21`, "html", background = bg.picker2(`Points 21`, max2), color = "white", tooltip = paste0("Predicted Point: ",`Points 21`)),
       `GW 22` = cell_spec(`GW 22`, "html", background = bg.picker2(`Points 22`, max3), color = "white", tooltip = paste0("Predicted Point: ",`Points 22`)),
       `GW 23` = cell_spec(`GW 23`, "html", background = bg.picker2(`Points 23`, max4), color = "white", tooltip = paste0("Predicted Point: ",`Points 23`)),
       `GW 24` = cell_spec(`GW 24`, "html", background = bg.picker2(`Points 24`, max5), color = "white", tooltip = paste0("Predicted Point: ",`Points 24`)),
...
table %>%
   mutate(
      ...
      Club = cell_spec(club, "html", background = sapply(table$club, bg.picker), color = "white"),
      Value = color_bar(sapply(table$club, bg.picker))(value),
      `GW 20` = color_tile("#ff0000", "#33cc33")(`GW 20`),
      `GW 21` = color_tile("#ff0000", "#33cc33")(`GW 21`),
      `GW 22` = color_tile("#ff0000", "#33cc33")(`GW 22`),
      `GW 23` = color_tile("#ff0000", "#33cc33")(`GW 23`),
      `GW 24` = color_tile("#ff0000", "#33cc33")(`GW 24`),
      Total = color_bar("#ff7f7f")(Total)
   ) %>%
   dplyr::select(Position, Player, Club, Value,`GW 20`, `GW 21`, `GW 22`, `GW 23`, `GW 24`, Total) %>%
   kable("html", escape = F,align = c("c", "c", "c", "r", "c", "c", "c","c" , "c", "r")) %>%
...
output$titletable <- renderText({"Gameweek Squad Table"})
...
output$playerpoints <- renderHighchart({
   ...
   table11 <- table %>% 
      ...
      highchart() %>%
      hc_title(text = "Your Team Performance", style = list(color = "#FFFF", fontSize = "18px", fontFamily = 'monospace')) %>% 
...
teamfilt <- reactive({
  ...
  table_points <- teamlist()
  
  playerlist <- c(input$PickGK, input$PickDF[1],input$PickDF[2],input$PickDF[3],input$PickMid[1],input$PickMid[2],input$PickMid[3],input$PickMid[4],
  input$PickFor[1],input$PickFor[2],input$PickFor[3])
...
output$card_transferout <- renderUI({
  ...
  team_filt <- team_filt %>% 
    filter(position == input$SelectPosition)
  ...
  total_points_former <- round(total_points_former,2)
  total_points_former <- paste0("Projected Points 5 GWs - ", total_points_former) 
  
  fpl_shots <- fpl_shots %>% 
    dplyr::filter(player %in% player_transfer_out) 
  ...
  HTML(
    paste0(
      '<div class="card3">
      <div class="container">
              <h4><b>Algorithm Transfer Out Suggestion</b></h4>
              </div>
              <img src=', unique_player,' style="width:120px;height:150px;" class = "center" />
              <div class="container">
              <h5><b>', player_transfer_out,'<b></h5>
              <h5>', total_points_former,'</h5>
              </div>
              </div>'
    )
  )
...
output$card_transferin <- renderUI({
  ...
  player_transfer_value <- team_filt[ which.min(team_filt$pred_total_points), 'value']
  total_points_former <- team_filt[ which.min(team_filt$pred_total_points), 'pred_total_points']
  player_former <- team_filt[ which.min(team_filt$pred_total_points), 'player']
  
  playerlist <- c(input$PickGK, input$PickDF[1],input$PickDF[2],input$PickDF[3],input$PickMid[1],input$PickMid[2],input$PickMid[3],input$PickMid[4],
                  input$PickFor[1],input$PickFor[2],input$PickFor[3]) 

  
  bank <- player_transfer_value + input$TeamValue
  ...
  playersearch <- table_points %>% 
    ...
    filter(position %in% input$SelectPosition) %>% 
  ...
  total_points_suggestion <- round(total_points_suggestion,2)
  different <- total_points_suggestion - total_points_former
  
  total_points_suggestion <- paste0("Projected Points 5 GWs - ", total_points_suggestion)
  
  fpl_shots <- fpl_shots %>% 
    dplyr::filter(player %in%  player_name_suggestion) 
  ...
  HTML(
    paste0(
      '<div class="card3">
      <div class="container">
              <h4><b>Algorithm Transfer In Suggestion</b></h4>
              </div>
              <img src=', unique_player,'  style="width:120px;height:150px;" class = "center" />
              <div class="container">
              <h5><b>', player_name_suggestion,' </b></h5>
              <h5>', total_points_suggestion,' </h5> 
              </div>
              </div>'
    )
  )
